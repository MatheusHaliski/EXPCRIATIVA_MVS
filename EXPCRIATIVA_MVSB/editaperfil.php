...
if ($result->num_rows > 0) {
    $row = $result->fetch_assoc();
    $nome_atual = $row['Nome'];
    $sobrenome_atual = $row['Sobrenome'];
    $email_atual = $row['Email'];
    $celular_atual = $row['Celular'];
    $genero_atual = $row['Genero'];
    $tipo_usuario = $row['usuarioadm'];
    ...
    echo "</tr>";
    echo "<tr>";
    echo "<td style='padding: 15px; width: 100px; font-size: larger;'>Tipo de Usuario:</td>";
    echo "<td style='padding: 10px; font-size: larger;'>";
    echo $row['usuarioadm'] == 1 ? 'Administrador' : 'Comum';
    echo "</td>";
    echo "</tr>";    
    echo "</table>";   
    echo "<form action='http://localhost/EXPCRIATIVA_MVSB-MAIN/updateperfil.php' method='POST' enctype='multipart/form-data'>";
      ...
      // Campos de entrada com valores padrão preenchidos
      echo "<tr><td style='padding: 10px;'>Novo Nome:</td><td style='padding: 10px;'><input type='text' name='nome' value='" . $nome_atual . "' style='width: 100%; padding: 5px; border-radius: 10px; border: 1px solid #333;'></td></tr>";
      echo "<tr><td style='padding: 10px;'>Novo Sobrenome:</td><td style='padding: 10px;'><input type='text' name='sobrenome' value='" . $sobrenome_atual . "' style='width: 100%; padding: 5px; border-radius: 10px; border: 1px solid #333;'></td></tr>";
      echo "<tr><td style='padding: 10px;'>Novo email:</td><td style='padding: 10px;'><input type='text' name='email' value='" . $email_atual . "' style='width: 100%; padding: 5px; border-radius: 10px; border: 1px solid #333;'></td></tr>";
      echo "<tr><td style='padding: 10px;'>Novo Celular:</td><td style='padding: 10px;'><input type='text' name='celular' value='" . $celular_atual . "' style='width: 100%; padding: 5px; border-radius: 10px; border: 1px solid #333;'></td></tr>";
      echo "<tr><td style='padding: 10px;'>Nova Senha:</td><td style='padding: 10px;'><input type='password' id='senha' name='senha' value='" . $senha_u  . "' style='width: 85%; padding: 5px; border-radius: 10px; border: 1px solid #333;'><button type='button' onclick='toggleSenha()' style='margin-left: 5px; padding: 5px 10px; background-color: #333; color: #fff; border: none; border-radius: 10px; cursor: pointer;'>Mostrar</button></td></tr>";
      ...
      echo "<option value='masculino' " . ($genero_atual == 'masculino' ? 'selected' : '') . ">Masculino</option>";
      echo "<option value='feminino' " . ($genero_atual == 'feminino' ? 'selected' : '') . ">Feminino</option>";
      echo "<option value='outros' " . ($genero_atual == 'outros' ? 'selected' : '') . ">Outros</option>";
      echo "<option value='nao_quero_dizer' " . ($genero_atual == 'nao_quero_dizer' ? 'selected' : '') . ">Não quero dizer</option>";
      echo "</select></td></tr>";
      echo "<tr><td style='padding: 10px;'>Tipo de Usuario:</td><td style='padding: 10px;'><select name='tipusuario' style='width: 100%; padding: 5px; border-radius: 10px; border: 1px solid #333;'>";
      echo "<option value='1' " . ($tipo_usuario == 'adm' ? 'selected' : '') . ">Administrador</option>";
      echo "<option value='0' " . ($tipo_usuario == 'comum' ? 'selected' : '') . ">Comum</option>";
